...
export function ThemeSwitcher() {
    const [isDarkTheme, setIsDarkTheme] = useState(store.settings.getLocal('dark-theme', false));

    useEffect(() => {
        const darkTheme = store.settings.getLocal('dark-theme', false);
    ...
    function onChange(event: React.ChangeEvent<HTMLInputElement>){
        ...
        store.settings.setLocal('dark-theme', event.target.checked);
        updateTheme(event.target.checked);
    }

    return (
        <label className="swap swap-rotate">
            <input type="checkbox" checked={isDarkTheme} onChange={onChange} />
